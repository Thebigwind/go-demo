...
const (
	Zhengdao_CLIENT_ADMIN_PASSFILE         = "/etc/.Zhengdao_admin.key"
	Zhengdao_INITIAL_ADMIN_PASSWORD string = "aaa"
	Zhengdao_ADMIN_PASSWORD_SEED    string = "bbbb" //
	Zhengdao_ADMIN_PASSWORD_SHADOW  string = "ccccc"
)
...
/*
 * encrypt admin passwd by Zhengdao_ADMIN_PASSWORD_SHADOW at local
 * encrypt admin passwd by Zhengdao_ADMIN_PASSWORD_SEED at ETCD
 * admin skey = encrypt admin passwd by Zhengdao_SECURITY_KEY_SEED
 */
...
func SaveAdminPassword(pass string) error {
	...
	aesPass := NewAesEncrypt(Zhengdao_ADMIN_PASSWORD_SHADOW)
	...
	err = os.Rename(tmpfile.Name(), Zhengdao_CLIENT_ADMIN_PASSFILE)
	if err != nil {
...
func LoadAdminPassword() (string, error) {
	_, err := os.Stat(Zhengdao_CLIENT_ADMIN_PASSFILE)
	if os.IsNotExist(err) {
		return "", nil
	}

	o, err := ioutil.ReadFile(Zhengdao_CLIENT_ADMIN_PASSFILE)
	if err != nil {
...
func LoadAdminSkey() (string, error) {
	...
	_, err := os.Stat(Zhengdao_CLIENT_ADMIN_PASSFILE)
	if os.IsNotExist(err) {
		return Zhengdao_INITIAL_ADMIN_PASSWORD, nil
	}
	...
	aesSkey := NewAesEncrypt(ZHENGDAO_SECURITY_KEY_SEED)
